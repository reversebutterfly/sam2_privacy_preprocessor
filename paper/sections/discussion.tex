...
The DAVIS main sweep uses $n=36$ of 90 videos; the full 90-video sweep is ongoing (parallel run). The YT-VOS operating-point experiment ($n=50$, ssim\_floor=0.90) reports AdvOpt alpha distributions confirming $94\%$ convergence to $\alpha \in [0.92, 0.93)$; the paired JF comparison at matched quality is pending completion.
...
The proxy (within-video $r=+0.811$, oracle gap 98\%) validates that AdvOpt reliably finds this optimal point under the quality constraint, rather than by exhaustive search.

On heterogeneous content (YT-VOS), $12\%$ of videos genuinely require lower $\alpha$ to satisfy the SSIM constraint.
For these videos, per-video selection via AdvOpt is necessary; a global fixed setting would violate the quality bound.
The proxy correctly identifies these constraint-bound cases.
...
\paragraph{Proxy scope.} The proxy and oracle-gap experiments are validated on DAVIS. YT-VOS proxy validation (within-video correlation) is not yet available.
